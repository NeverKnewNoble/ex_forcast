<template>
  <div class="flex flex-col items-center justify-center min-h-[400px] bg-white border-2 border-dashed border-violet-300 rounded-xl shadow-sm">
    ...
    <p class="text-gray-500 text-center max-w-md leading-relaxed text-sm">
      {{ fromYear && !toYear
        ? 'You have selected a From Year, now please select a To Year to display the Receipts & Payments table.'
        : !fromYear && toYear
          ? 'You have selected a To Year, now please select a From Year to display the Receipts & Payments table.'
          : 'Please select both "From Year" and "To Year" to display the Receipts & Payments table.' }}
    </p>
    <div class="mt-4 flex items-center gap-2 text-xs text-violet-600">
      <ArrowLeft class="w-3 h-3" />
      <span>Use the filters to get started</span>
    </div>
    <div class="mt-6">
      <router-link
        to="/expense_estimate"
